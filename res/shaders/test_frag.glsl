...
uniform DirectionLight directionLights[32];
...
vec3 CalcuDirectionLight(DirectionLight light, vec3 normal, vec3 viewDir);
...
void main() {
  ...
  for (int i = 0; i < directionLightCount; ++i) {
    result += CalcuDirectionLight(directionLights[i], norm, viewDir);
  }
  for (int i = 0; i < pointLightCount; ++i) {
    result += CalcuPointLight(pointLights[i], norm, FragPos, viewDir);
  }
  for (int i = 0; i < spotLightCount; ++i) {
    result += CalcuSpotLight(spotLights[i], norm, FragPos, viewDir);
  }
...
vec3 CalcuPointLight(PointLight light, vec3 normal, vec3 fragPos,
                     vec3 viewDir) {
