...
SSAO::SSAO(int width, int height) {
  ...
  width_  = width;
  height_ = height;
  // SSAO runs at half resolution for performance; the box blur smooths it.
  ssao_width_  = std::max(1, width_ / 2);
  ssao_height_ = std::max(1, height_ / 2);
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, ssao_width_, ssao_height_, 0, GL_RGBA, GL_FLOAT, nullptr);
  ...
  glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, ssao_width_, ssao_height_);
  ...
  for (int i = 0; i < sample_count_; ++i) {
    ...
    const float scale = Lerp(0.1f, 1.0f, std::pow(static_cast<float>(i) / sample_count_, 2.0f));
    sample *= scale;
...
void SSAO::Generate(const glm::mat4 &proj, const glm::mat4 &view) const {
  ...
  ssao_shader_->SetUniform("noise_scale", glm::vec2(static_cast<float>(ssao_width_) / 4.0f,
                                                    static_cast<float>(ssao_height_) / 4.0f));
